...
for my $chrom (sort {if ($a=~/^\d+$/ && $b=~/^\d+$/){$a<=>$b} else { if ($a eq "MT") {return 1} elsif($b eq "MT") {return -1}else{$a cmp $b} }} keys(%intervalsByChrom))
{
...
for my $chrom (keys(%intervalsByChrom))
{
...
##########################################
##Concatenate sites only for VQSR training
##########################################
#push(@tgts,"$finalVCFOutDir/all.sites.vcf.gz.OK");
#push(@deps," $chromVCFIndicesOK");
#$cmd = "\t$vt concat -L $finalVCFOutDir/merge_vcf_list.txt -s -o $finalVCFOutDir/all.sites.vcf.gz 2> $finalVCFOutDir/merge_vcfs.log\n";
#$cmd = "\t" . $cmd . "\n";
#$cmd .= "\n\ttouch $finalVCFOutDir/all.sites.vcf.gz.OK\n";
#push(@cmds, $cmd);
